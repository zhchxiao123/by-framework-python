...
_CATALOG: dict[str, MetricDefinition] = {
    ...
    "by_framework_execution_status_total": MetricDefinition(
        ...
        description="Legacy terminal execution count by status, agent type, and worker.",
        interpretation="Debug-only worker-level execution count; avoid long-retention aggregation by worker_id.",
        debug_only=True,
    ),
    "by_framework_execution_latency_ms": MetricDefinition(
        ...
        interpretation="Debug-only worker-level latency; prefer by_framework_execution_run_duration_seconds.",
        debug_only=True,
    ),
    "by_framework_queue_wait_ms": MetricDefinition(
        ...
        interpretation="Compatibility metric; prefer by_framework_execution_queue_duration_seconds.",
    ),
    "by_framework_availability_routing_ms": MetricDefinition(
        ...
        description="Availability routing latency before message dispatch in milliseconds.",
    ...
    "by_framework_stream_max_delivery_count": MetricDefinition(
        ...
        interpretation="Retry pressure signal: high values indicate messages repeatedly redelivered before ack.",
    ),
    ...
    "by_framework_execution_total_latency_p95_ms": MetricDefinition(
        ...
        interpretation="Legacy user-visible latency signal; prefer seconds duration metrics.",
    ),
    "by_framework_execution_queue_duration_p95_seconds": MetricDefinition(
        ...
        interpretation="Latency signal for queue wait over the retained snapshot window.",
    ),
    "by_framework_execution_run_duration_p95_seconds": MetricDefinition(
        ...
        interpretation="Latency signal for worker processing over the retained snapshot window.",
    ),
    ...
    "by_framework_observability_export_failures_by_exporter_total": MetricDefinition(
        ...
        interpretation="Instrumentation reliability signal pinpointing exporter failures.",
    ),
}
...
def get_metric_catalog_payload() -> dict[str, object]:
    ...
    debug_count = sum(
        1 for definition in _CATALOG.values() if definition.debug_only
    )
    return {
        "total": len(metrics),
